...
const addMsgIDInMsgsArrayOfConversation = async (conversation: IConversation, createdMessage: IMessage) => {
...
const getConversation = async (receiverId: string, senderId: string) => {
    ...
    const conversation = await _chatRepository.getConversation(
        receiverId,
        senderId
    );

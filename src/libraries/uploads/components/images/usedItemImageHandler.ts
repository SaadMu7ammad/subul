...
const resizeImg = async (
  ...
  try {
    if (Array.isArray(req.files) && !req.files.length) throw new BadRequestError('no image uploaded');

    let  destinationFolder = 'usedItemsImages', suffix = 'usedItem-';

    if(Array.isArray(req.files)){//type guard
        ...
        req.body.images = Array.isArray(req.body.images) ? [...req.body.images,fileName] : [fileName];
      }
    } 
    next();
  } catch (err) {
